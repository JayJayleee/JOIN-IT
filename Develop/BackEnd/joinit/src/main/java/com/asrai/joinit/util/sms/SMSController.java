package com.asrai.joinit.util.sms;

import com.asrai.joinit.domain.PhoneCertCode;
import com.asrai.joinit.user.UserService;
...
@RequestMapping("/sms")
public class SMSController implements InitializingBean {
...
//    }
    ...
    @ResponseBody
    public SingleMessageSentResponse sendOne(@RequestBody PhoneCertCode phoneCertCode) {

        log.info(phoneCertCode.getPhone());
        String phone = phoneCertCode.getPhone();
        String certCode = userService.createPhoneCertCode(phone);

        Message message = new Message();
        // 발신번호 및 수신번호는 반드시 01012345678 형태로 입력되어야 합니다.
        message.setFrom(CALLING_NUMBER);
        message.setTo(phoneCertCode.getPhone());
        message.setText("[JOIN-IT 본인인증]\n 인증번호는 [" + certCode + "] 입니다.");
