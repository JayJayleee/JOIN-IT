...
public class CustomOAuth2UserService implements OAuth2UserService<OAuth2UserRequest, OAuth2User> {
    ...
    private User saveUser(OAuthAttributes attributes, SocialType socialType) {
        ...
        userSequence.setPatientSeq(lastNumber);
        userSequence.setTherapistSeq(userSequenceRepository.findLastSeqTherapists());
        userSequenceRepository.save(userSequence);

        userId += lastNumber;

        Patient createdUser = attributes.toEntity(userId, socialType, attributes.getOauth2UserInfo());
        log.info("createdUser : {}", createdUser);

        patientRepository.save(createdUser);
        return userRepository.findByUserId(createdUser.getUserId()).get();
    }
}