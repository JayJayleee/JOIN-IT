...
function CompleteCare({care, eventChangeFtn, setTreatId, setCompleteCareStatus, setNameAndJoint}: propType) {
  ...
  return (
    <React.Fragment>
      <div className='row profile-carelist-detail care-complete'>
        ...
        <div className='col profile-carelist-detail-text'>
          <div className='row profile-carelist-detail-text-title'>
            ...
            <p>{care.startTime !== undefined? care.startTime.substring(2,10).replaceAll('-', '.') : null} - {care.endTime !== null? care.endTime.substring(2,10).replaceAll('-', '.') : null}</p>
          </div>
          ...
          <div className='row profile-carelist-detail-text-description'>
            ...
            <div className='profile-carelist-detail-button care-completeBtn' onClick={(e) => {mixFnt(e)}}>
              <p>자세히 보기</p>
            </div>
          </div>
        </div>
